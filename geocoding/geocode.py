...
READFROM = 'new_spaces.csv'
WRITETO = 'spaces_new_latlng.csv'
	...
	with open(WRITETO, 'wb') as fileToWrite:
		...
		for row in reader:
			if counter==0:
				...
				if row[headers.index("Lat")] == None or row[headers.index("Long")] == None or row[headers.index("Lat")].strip() == "" or row[headers.index("Long")].strip() == "":


					place = urllib.urlencode({'query':str(row[3])+", "+str(row[5])+", "+str(row[2])})
